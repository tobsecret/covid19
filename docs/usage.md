...
First, go to the [nf-core/covid19 releases page](https://github.com/nf-core/covid19/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from dockerhub: [`nfcore/covid19`](http://hub.docker.com/r/nfcore/covid19/)
* `singularity`
